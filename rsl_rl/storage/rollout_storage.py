...
class RolloutStorage:
    ...
    def __init__(
        ...
        # For RNN networks
        self.saved_hidden_states = torch.zeros(num_transitions_per_env, num_envs, *hidden_states_shape, device=self.device)
    ...
    def add_transitions(self, transition: Transition):
        ...
        # For GRU terrain encoder
        if transition.hidden_states is not None:
            self.saved_hidden_states[self.step].copy_(transition.hidden_states)
    ...
    def mini_batch_generator(self, num_mini_batches, num_epochs=8):
        ...
        saved_hidden_states = self.saved_hidden_states.flatten(0, 1)
        # For RND
        if self.rnd_state_shape is not None:
        ...
        for epoch in range(num_epochs):
            for i in range(num_mini_batches):
                ...
                hid_states_batch = saved_hidden_states[batch_idx]
                # -- For RND
                if self.rnd_state_shape is not None:
                ...
                yield obs_batch, privileged_observations_batch, actions_batch, target_values_batch, advantages_batch, returns_batch, old_actions_log_prob_batch, old_mu_batch, old_sigma_batch, hid_states_batch, None, rnd_state_batch